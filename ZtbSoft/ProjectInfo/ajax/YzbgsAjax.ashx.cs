...
namespace ZtbSoft.ProjectInfo.ajax
{
    ...
    /// Zbwj 的摘要说明
    /// </summary>
    public class YzbgsAjax : IHttpHandler, IRequiresSessionState
    {
        ...
        public void GetList(HttpContext context)
        {
            ...
            Hashtable result = new Hashtable();
            string noticeType = enum_NoticeType.Yzbgs.ToString();
            if (context.Request["pid"] != null)
            {
                string projectId = context.Request["pid"];
                result["data"] = new SFirstResultBLL().GetResultListByProjectInfoId(projectId);
            }
            //JSON
            String json = JsonHelper.Encode(result);
            context.Response.ContentType = "text/plain";
