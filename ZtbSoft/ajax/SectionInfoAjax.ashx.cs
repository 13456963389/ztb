...
namespace ZtbSoft.Web.Ajax
{
    ...
    /// SectionInfo 标段信息
    /// </summary>
    public class SectionInfoAjax : IHttpHandler,IRequiresSessionState
    {
        ...
        public void GetList(HttpContext context)
        {
            ...
            result["data"] = new SectionInfoBLL().GetPage(sel_col, sel_whe, ord, pageSize, pageIndex, out count);
            result["total"] = count;
        ...
        public void Save(HttpContext context)
        {
            ...
            foreach (Hashtable row in rows)
            {
                ...
                String id = row["SectionId"] != null ? row["SectionId"].ToString() : "";
                ...
                if (state == "added" || id == "")           //新增：id为空，或_state为added
                {
                    ...
                    row["SectionId"]=-1;
                    count += new SectionInfoBLL().Insert(JsonHelper.Encode(row));
                }
                else if (state == "removed" || state == "deleted")  //删除
                {
                    ...
                    count += new SectionInfoBLL().DeleteBySectionId(Convert.ToInt32(id));
                }
                else if (state == "modified" || state == "") //更新：_state为空或modified
                {
                    ...
                    count += new SectionInfoBLL().Update(JsonHelper.Encode(row));
                }
            }
        ...
        public void Remove(HttpContext context)
        {
            ...
            {
                string id = ids[i];
                new SectionInfoBLL().DeleteBySectionId(Convert.ToInt32(id));
            }
        }
        ...
        public void GetModel(HttpContext context)
        {
            ...
            SectionInfo sectionInfo = new SectionInfoBLL().GetBySectionId(Convert.ToInt32(id));
            String json = JsonHelper.Encode(sectionInfo);
            context.Response.Write(json);
        }
