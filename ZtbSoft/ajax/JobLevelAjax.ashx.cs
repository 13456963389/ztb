...
namespace ZtbSoft.Web.ajax
{
    ...
    /// JobLevel 职别代码表
    /// </summary>
    public class JobLevelAjax : IHttpHandler, IRequiresSessionState
    {

        private DataTable dtPower = null;   //当前用户权限
        private string powerCode = enum_PowerObjectCode.JobLevel.ToString();
        ...
        public void GetList(HttpContext context)
        {
            ...
            //查询字段
            string JobLevelName = context.Request["JobLevelName"];
            ...
            { sel_whe += " and JobLevelName like '%" + JobLevelName + "%'"; }
            ...
            result["data"] = new JobLevelBLL().GetPage(sel_col, sel_whe, ord, pageSize, pageIndex, out count);
            result["total"] = count;
        ...
        public void Save(HttpContext context)
        {
            ...
            foreach (Hashtable row in rows)
            {
                ...
                String id = row["JobLevelId"] != null ? row["JobLevelId"].ToString() : "";
                ...
                if (state == "added" || id == "")           //新增：id为空，或_state为added
                {
                    ...
                    row["JobLevelId"] = -1;
                    count += new JobLevelBLL().Insert(JsonHelper.Encode(row));
                }
                else if (state == "removed" || state == "deleted")  //删除
                {
                    ...
                    count += new JobLevelBLL().DeleteByJobLevelId(Convert.ToInt32(id));
                }
                else if (state == "modified" || state == "") //更新：_state为空或modified
                {
                    ...
                    count += new JobLevelBLL().Update(JsonHelper.Encode(row));
                }
            }
        ...
        public void Remove(HttpContext context)
        {
            ...
            {
                string id = ids[i];
                new JobLevelBLL().DeleteByJobLevelId(Convert.ToInt32(id));
            }
        }
        ...
        public void GetModel(HttpContext context)
        {
            ...
            JobLevel jobLevel = new JobLevelBLL().GetByJobLevelId(Convert.ToInt32(id));
            String json = JsonHelper.Encode(jobLevel);
            context.Response.Write(json);
        }
