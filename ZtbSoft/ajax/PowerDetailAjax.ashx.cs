...
namespace ZtbSoft.Web.ajax
{
    ...
    /// PowerDetail 操作权限代码
    /// </summary>
    public class PowerDetailAjax : IHttpHandler,IRequiresSessionState
    {
		
        private DataTable dtPower = null;   //当前用户权限
        private string powerCode = enum_PowerObjectCode.PowerDetail.ToString();
        ...
        public void GetList(HttpContext context)
        {
            ...
            //查询字段
			string PowerDetailName = context.Request["PowerDetailName"];
			...
			//查询条件
			if (!string.IsNullOrWhiteSpace(PowerDetailName))
{  sel_whe += " and PowerDetailName like '%" + PowerDetailName + "%'";  }
            ...
            result["data"] = new PowerDetailBLL().GetPage(sel_col, sel_whe, ord, pageSize, pageIndex, out count);
            result["total"] = count;
        ...
        public void Save(HttpContext context)
        {
            ...
            foreach (Hashtable row in rows)
            {
                ...
                String id = row["PowerDetailId"] != null ? row["PowerDetailId"].ToString() : "";
                ...
                if (state == "added" || id == "")           //新增：id为空，或_state为added
                {
                    ...
                    row["PowerDetailId"]=-1;
                    count += new PowerDetailBLL().Insert(JsonHelper.Encode(row));
                }
                else if (state == "removed" || state == "deleted")  //删除
                {
                    ...
                    count += new PowerDetailBLL().DeleteByPowerDetailId(Convert.ToInt32(id));
                }
                else if (state == "modified" || state == "") //更新：_state为空或modified
                {
                    ...
                    count += new PowerDetailBLL().Update(JsonHelper.Encode(row));
                }
            }
        ...
        public void Remove(HttpContext context)
        {
            ...
            {
                string id = ids[i];
                new PowerDetailBLL().DeleteByPowerDetailId(Convert.ToInt32(id));
            }
        }
        ...
        public void GetModel(HttpContext context)
        {
            ...
            PowerDetail powerDetail = new PowerDetailBLL().GetByPowerDetailId(Convert.ToInt32(id));
            String json = JsonHelper.Encode(powerDetail);
            context.Response.Write(json);
        }
