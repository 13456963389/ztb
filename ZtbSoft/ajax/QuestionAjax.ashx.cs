...
namespace ZtbSoft.Web.Ajax
{
    ...
    /// Question 异议表
    /// </summary>
    public class QuestionAjax : IHttpHandler,IRequiresSessionState
    {
		
        private DataTable dtPower = null;   //当前用户权限
        private string powerCode = enum_PowerObjectCode.Question.ToString();
        ...
        public void GetList(HttpContext context)
        {
            ...
            result["data"] = new QuestionBLL().GetPage(sel_col, sel_whe, ord, pageSize, pageIndex, out count);
            result["total"] = count;
        ...
        public void Save(HttpContext context)
        {
            ...
            foreach (Hashtable row in rows)
            {
                ...
                String id = row["QuestionId"] != null ? row["QuestionId"].ToString() : "";
                ...
                if (state == "added" || id == "")           //新增：id为空，或_state为added
                {
                    ...
                    row["QuestionId"]=-1;
                    count += new QuestionBLL().Insert(JsonHelper.Encode(row));
                }
                else if (state == "removed" || state == "deleted")  //删除
                {
                    ...
                    count += new QuestionBLL().DeleteByQuestionId(Convert.ToInt32(id));
                }
                else if (state == "modified" || state == "") //更新：_state为空或modified
                {
                    ...
                    count += new QuestionBLL().Update(JsonHelper.Encode(row));
                }
            }
        ...
        public void Remove(HttpContext context)
        {
            ...
            {
                string id = ids[i];
                new QuestionBLL().DeleteByQuestionId(Convert.ToInt32(id));
            }
        }
        ...
        public void GetModel(HttpContext context)
        {
            ...
            Question question = new QuestionBLL().GetByQuestionId(Convert.ToInt32(id));
            String json = JsonHelper.Encode(question);
            context.Response.Write(json);
        }
