...
namespace ZtbSoft.Web.Ajax
{
    ...
    /// UpdateLog 更新日志
    /// </summary>
    public class UpdateLogAjax : IHttpHandler, IRequiresSessionState
    {

        private DataTable dtPower = null;   //当前用户权限
        private string powerCode = enum_PowerObjectCode.UpdateLog.ToString();
        ...
        public void GetListShow(HttpContext context)
        {
            DataTable dt = new ZtbSoft.DAL.UpdateLogDAL().GetTop(10, "[UpdateLogId],[Title],[Version],[Time],[Description]", "", "[time] desc");
            //JSON
        ...
        public void GetList(HttpContext context)
        {
            ...
            result["data"] = new UpdateLogBLL().GetPage(sel_col, sel_whe, ord, pageSize, pageIndex, out count);
            result["total"] = count;
        ...
        public void Save(HttpContext context)
        {
            ...
            foreach (Hashtable row in rows)
            {
                ...
                String id = row["UpdateLogId"] != null ? row["UpdateLogId"].ToString() : "";
                ...
                if (state == "added" || id == "")           //新增：id为空，或_state为added
                {
                    ...
                    row["UpdateLogId"] = -1;
                    count += new UpdateLogBLL().Insert(JsonHelper.Encode(row));
                }
                else if (state == "removed" || state == "deleted")  //删除
                {
                    ...
                    count += new UpdateLogBLL().DeleteByUpdateLogId(Convert.ToInt32(id));
                }
                else if (state == "modified" || state == "") //更新：_state为空或modified
                {
                    ...
                    count += new UpdateLogBLL().Update(JsonHelper.Encode(row));
                }
            }
        ...
        public void Remove(HttpContext context)
        {
            ...
            {
                string id = ids[i];
                new UpdateLogBLL().DeleteByUpdateLogId(Convert.ToInt32(id));
            }
        }
        ...
        public void GetModel(HttpContext context)
        {
            ...
            UpdateLog updateLog = new UpdateLogBLL().GetByUpdateLogId(Convert.ToInt32(id));
            String json = JsonHelper.Encode(updateLog);
            context.Response.Write(json);
        }
        ...
        /// 获取实体
        /// </summary>
        public void GetModelShow(HttpContext context)
        {            
            string id = context.Request["id"];
            UpdateLog updateLog = new UpdateLogBLL().GetByUpdateLogId(Convert.ToInt32(id));
            String json = JsonHelper.Encode(updateLog);
