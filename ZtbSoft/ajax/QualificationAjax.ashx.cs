...
namespace ZtbSoft.Web.Ajax
{
    ...
    /// Qualification 资质信息
    /// </summary>
    public class QualificationAjax : IHttpHandler,IRequiresSessionState
    {
		
        private DataTable dtPower = null;   //当前用户权限
        private string powerCode = enum_PowerObjectCode.Qualification.ToString();
        ...
        public void GetList(HttpContext context)
        {
            ...
            result["data"] = new QualificationBLL().GetPage(sel_col, sel_whe, ord, pageSize, pageIndex, out count);
            result["total"] = count;
        ...
        public void Save(HttpContext context)
        {
            ...
            foreach (Hashtable row in rows)
            {
                ...
                String id = row["UQId"] != null ? row["UQId"].ToString() : "";
                ...
                if (state == "added" || id == "")           //新增：id为空，或_state为added
                {
                    ...
                    row["UQId"]=-1;
                    count += new QualificationBLL().Insert(JsonHelper.Encode(row));
                }
                else if (state == "removed" || state == "deleted")  //删除
                {
                    ...
                    count += new QualificationBLL().DeleteByUQId(Convert.ToInt32(id));
                }
                else if (state == "modified" || state == "") //更新：_state为空或modified
                {
                    ...
                    count += new QualificationBLL().Update(JsonHelper.Encode(row));
                }
            }
        ...
        public void Remove(HttpContext context)
        {
            ...
            {
                string id = ids[i];
                new QualificationBLL().DeleteByUQId(Convert.ToInt32(id));
            }
        }
        ...
        public void GetModel(HttpContext context)
        {
            ...
            Qualification qualification = new QualificationBLL().GetByUQId(Convert.ToInt32(id));
            String json = JsonHelper.Encode(qualification);
            context.Response.Write(json);
        }
