...
namespace ZtbSoft.Web.Ajax
{
    ...
    public class NoticeAjax : IHttpHandler,IRequiresSessionState
    {
        ...
        private string powerCode = enum_PowerObjectCode.Notice.ToString();
        ...
        public void GetList(HttpContext context)
        {
            ...
            result["data"] = new NoticeBLL().GetPage(sel_col, sel_whe, ord, pageSize, pageIndex, out count);
            result["total"] = count;
        ...
        public void Save(HttpContext context)
        {
            ...
            foreach (Hashtable row in rows)
            {
                ...
                String id = row["NoticeId"] != null ? row["NoticeId"].ToString() : "";
                ...
                if (state == "added" || id == "")           //新增：id为空，或_state为added
                {
                    ...
                    row["NoticeId"]=-1;
                    count += new NoticeBLL().Insert(JsonHelper.Encode(row));
                }
                else if (state == "removed" || state == "deleted")  //删除
                {
                    ...
                    count += new NoticeBLL().DeleteByNoticeId(Convert.ToInt32(id));
                }
                else if (state == "modified" || state == "") //更新：_state为空或modified
                {
                    ...
                    count += new NoticeBLL().Update(JsonHelper.Encode(row));
                }
            }
        ...
        public void Remove(HttpContext context)
        {
            ...
            {
                string id = ids[i];
                new NoticeBLL().DeleteByNoticeId(Convert.ToInt32(id));
            }
        }
        ...
        public void GetModel(HttpContext context)
        {
            ...
            Notice notice = new NoticeBLL().GetByNoticeId(Convert.ToInt32(id));
            String json = JsonHelper.Encode(notice);
            context.Response.Write(json);
        }
