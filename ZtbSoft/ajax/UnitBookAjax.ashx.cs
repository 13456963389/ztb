...
namespace ZtbSoft.Web.Ajax
{
    ...
    /// UnitBook 企业证书信息
    /// </summary>
    public class UnitBookAjax : IHttpHandler,IRequiresSessionState
    {
		
        private DataTable dtPower = null;   //当前用户权限
        private string powerCode = enum_PowerObjectCode.UnitBook.ToString();
        ...
        public void GetList(HttpContext context)
        {
            ...
            result["data"] = new UnitBookBLL().GetPage(sel_col, sel_whe, ord, pageSize, pageIndex, out count);
            result["total"] = count;
        ...
        public void Save(HttpContext context)
        {
            ...
            foreach (Hashtable row in rows)
            {
                ...
                String id = row["UnitBookId"] != null ? row["UnitBookId"].ToString() : "";
                ...
                if (state == "added" || id == "")           //新增：id为空，或_state为added
                {
                    ...
                    row["UnitBookId"]=-1;
                    count += new UnitBookBLL().Insert(JsonHelper.Encode(row));
                }
                else if (state == "removed" || state == "deleted")  //删除
                {
                    ...
                    count += new UnitBookBLL().DeleteByUnitBookId(Convert.ToInt32(id));
                }
                else if (state == "modified" || state == "") //更新：_state为空或modified
                {
                    ...
                    count += new UnitBookBLL().Update(JsonHelper.Encode(row));
                }
            }
        ...
        public void Remove(HttpContext context)
        {
            ...
            {
                string id = ids[i];
                new UnitBookBLL().DeleteByUnitBookId(Convert.ToInt32(id));
            }
        }
        ...
        public void GetModel(HttpContext context)
        {
            ...
            UnitBook unitBook = new UnitBookBLL().GetByUnitBookId(Convert.ToInt32(id));
            String json = JsonHelper.Encode(unitBook);
            context.Response.Write(json);
        }
