...
namespace ZtbSoft.Web.Ajax
{
    ...
    /// ExpertInfo 评标专家信息
    /// </summary>
    public class ExpertInfoAjax : IHttpHandler, IRequiresSessionState
    {

        private DataTable dtPower = null;   //当前用户权限
        private string powerCode = enum_PowerObjectCode.ExpertInfo.ToString();
        ...
        public void GetList(HttpContext context)
        {
            ...
            //查询字段
            string ExpertName = context.Request["ExpertName"];
            ...
            { sel_whe += " and ExpertName like '%" + ExpertName + "%'"; }
            ...
            result["data"] = new ExpertInfoBLL().GetPage(sel_col, sel_whe, ord, pageSize, pageIndex, out count);
            result["total"] = count;
        ...
        public void Save(HttpContext context)
        {
            ...
            foreach (Hashtable row in rows)
            {
                ...
                String id = row["ExpertId"] != null ? row["ExpertId"].ToString() : "";
                ...
                if (state == "added" || id == "")           //新增：id为空，或_state为added
                {
                    ...
                    row["ExpertId"] = -1;
                    count += new ExpertInfoBLL().Insert(JsonHelper.Encode(row));
                }
                else if (state == "removed" || state == "deleted")  //删除
                {
                    ...
                    count += new ExpertInfoBLL().DeleteByExpertId(Convert.ToInt32(id));
                }
                else if (state == "modified" || state == "") //更新：_state为空或modified
                {
                    ...
                    count += new ExpertInfoBLL().Update(JsonHelper.Encode(row));
                }
            }
        ...
        public void Remove(HttpContext context)
        {
            ...
            {
                string id = ids[i];
                new ExpertInfoBLL().DeleteByExpertId(Convert.ToInt32(id));
            }
        }
        ...
        public void GetModel(HttpContext context)
        {
            ...
            ExpertInfo expertInfo = new ExpertInfoBLL().GetByExpertId(Convert.ToInt32(id));
            String json = JsonHelper.Encode(expertInfo);
            context.Response.Write(json);
        }
