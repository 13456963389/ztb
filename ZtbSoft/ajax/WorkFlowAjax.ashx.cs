...
namespace ZtbSoft.Web.Ajax
{
    ...
    /// WorkFlow 工作流
    /// </summary>
    public class WorkFlowAjax : IHttpHandler,IRequiresSessionState
    {
		
        private DataTable dtPower = null;   //当前用户权限
        private string powerCode = enum_PowerObjectCode.WorkFlow.ToString();
        ...
        public void GetList(HttpContext context)
        {
            ...
            result["data"] = new WorkFlowBLL().GetPage(sel_col, sel_whe, ord, pageSize, pageIndex, out count);
            result["total"] = count;
        ...
        public void Save(HttpContext context)
        {
            ...
            foreach (Hashtable row in rows)
            {
                ...
                String id = row["CaseId"] != null ? row["CaseId"].ToString() : "";
                ...
                if (state == "added" || id == "")           //新增：id为空，或_state为added
                {
                    ...
                    row["CaseId"]=-1;
                    count += new WorkFlowBLL().Insert(JsonHelper.Encode(row));
                }
                else if (state == "removed" || state == "deleted")  //删除
                {
                    ...
                    count += new WorkFlowBLL().DeleteByCaseId(Convert.ToInt32(id));
                }
                else if (state == "modified" || state == "") //更新：_state为空或modified
                {
                    ...
                    count += new WorkFlowBLL().Update(JsonHelper.Encode(row));
                }
            }
        ...
        public void Remove(HttpContext context)
        {
            ...
            {
                string id = ids[i];
                new WorkFlowBLL().DeleteByCaseId(Convert.ToInt32(id));
            }
        }
        ...
        public void GetModel(HttpContext context)
        {
            ...
            WorkFlow workFlow = new WorkFlowBLL().GetByCaseId(Convert.ToInt32(id));
            String json = JsonHelper.Encode(workFlow);
            context.Response.Write(json);
        }
