...
namespace ZtbSoft.Web.Ajax
{
    ...
    /// FirstResult 资格预审结果
    /// </summary>
    public class FirstResultAjax : IHttpHandler,IRequiresSessionState
    {
		
        private DataTable dtPower = null;   //当前用户权限
        private string powerCode = enum_PowerObjectCode.FirstResult.ToString();
        ...
        public void GetList(HttpContext context)
        {
            ...
            result["data"] = new FirstResultBLL().GetPage(sel_col, sel_whe, ord, pageSize, pageIndex, out count);
            result["total"] = count;
        ...
        public void Save(HttpContext context)
        {
            ...
            foreach (Hashtable row in rows)
            {
                ...
                String id = row["FirstResultId"] != null ? row["FirstResultId"].ToString() : "";
                ...
                if (state == "added" || id == "")           //新增：id为空，或_state为added
                {
                    ...
                    row["FirstResultId"]=-1;
                    count += new FirstResultBLL().Insert(JsonHelper.Encode(row));
                }
                else if (state == "removed" || state == "deleted")  //删除
                {
                    ...
                    count += new FirstResultBLL().DeleteByFirstResultId(Convert.ToInt32(id));
                }
                else if (state == "modified" || state == "") //更新：_state为空或modified
                {
                    ...
                    count += new FirstResultBLL().Update(JsonHelper.Encode(row));
                }
            }
        ...
        public void Remove(HttpContext context)
        {
            ...
            {
                string id = ids[i];
                new FirstResultBLL().DeleteByFirstResultId(Convert.ToInt32(id));
            }
        }
        ...
        public void GetModel(HttpContext context)
        {
            ...
            FirstResult firstResult = new FirstResultBLL().GetByFirstResultId(Convert.ToInt32(id));
            String json = JsonHelper.Encode(firstResult);
            context.Response.Write(json);
        }
