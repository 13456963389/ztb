...
{
    /// <summary>
    /// NoticeTemplateAjax 的摘要说明
    /// </summary>
    public class NoticeTemplateAjax : IHttpHandler, IRequiresSessionState
    {
        private DataTable dtPower = null;   //当前用户权限
        private string powerCode = enum_PowerObjectCode.NoticeTemplate.ToString();
        ...
        public void GetList(HttpContext context)
        {
            ...
            result["data"] = new SSNoticeTemplateBLL().GetNoticeTemplateList();
        ...
        public void Save(HttpContext context)
        {
            ...
            string ctxt = context.Request["ctxt"];
            ArrayList rows = (ArrayList)JsonHelper.Decode(json);

            foreach (Hashtable row in rows)
            {

                String id = row["TemplateId"] != null ? row["TemplateId"].ToString() : "";
                ...
                if (state == "added" || id == "")           //新增：id为空，或_state为added
                {
                    ...
                    row["TemplateId"] = -1;
                    row["Context"] = ctxt;
                    count += new FlowTemplateBLL().Insert(JsonHelper.Encode(row));
                }
                else if (state == "removed" || state == "deleted")  //删除
                {
                    ...
                    count += new FlowTemplateBLL().DeleteByTemplateId(Convert.ToInt32(id));
                }
                else if (state == "modified" || state == "") //更新：_state为空或modified
                {
                    ...
                    row["Context"] = ctxt;
                    count += new FlowTemplateBLL().Update(JsonHelper.Encode(row));
                }
            }
        ...
        public void Remove(HttpContext context)
        {
            ...
            {
                string id = ids[i];
                new FlowTemplateBLL().DeleteByTemplateId(Convert.ToInt32(id));
            }
        }
        ...
        public void GetModel(HttpContext context)
        {
            ...
            FlowTemplate flowTemplate = new FlowTemplateBLL().GetByTemplateId(Convert.ToInt32(id));
            String json = JsonHelper.Encode(flowTemplate);
            context.Response.Write(json);
        }
