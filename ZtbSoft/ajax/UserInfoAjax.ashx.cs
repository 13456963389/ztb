...
namespace ZtbSoft.Web.Ajax
{
    ...
    public class UserInfoAjax : IHttpHandler,IRequiresSessionState
    {
        ...
        private string powerCode = enum_PowerObjectCode.UserInfo.ToString();
        ...
        public void GetList(HttpContext context)
        {
            ...
            result["data"] = new UserInfoBLL().GetPage(sel_col, sel_whe, ord, pageSize, pageIndex, out count);
            result["total"] = count;
        ...
        public void Save(HttpContext context)
        {
            ...
            foreach (Hashtable row in rows)
            {
                ...
                String id = row["UserId"] != null ? row["UserId"].ToString() : "";
                ...
                if (state == "added" || id == "")           //新增：id为空，或_state为added
                {
                    ...
                    row["UserId"]=-1;
                    count += new UserInfoBLL().Insert(JsonHelper.Encode(row));
                }
                else if (state == "removed" || state == "deleted")  //删除
                {
                    ...
                    count += new UserInfoBLL().DeleteByUserId(Convert.ToInt32(id));
                }
                else if (state == "modified" || state == "") //更新：_state为空或modified
                {
                    ...
                    count += new UserInfoBLL().Update(JsonHelper.Encode(row));
                }
            }
        ...
        public void Remove(HttpContext context)
        {
            ...
            {
                string id = ids[i];
                new UserInfoBLL().DeleteByUserId(Convert.ToInt32(id));
            }
        }
        ...
        public void GetModel(HttpContext context)
        {
            ...
            UserInfo userInfo = new UserInfoBLL().GetByUserId(Convert.ToInt32(id));
            String json = JsonHelper.Encode(userInfo);
            context.Response.Write(json);
        }
