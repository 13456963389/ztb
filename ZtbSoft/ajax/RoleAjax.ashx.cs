...
namespace ZtbSoft.Web.ajax
{
    ...
    public class RoleAjax : IHttpHandler,IRequiresSessionState
    {
        ...
        private string powerCode = enum_PowerObjectCode.Role.ToString();
...
{  sel_whe += " and RoleName like '%" + RoleName + "%'";  }
            ...
            result["data"] = new RoleBLL().GetPage(sel_col, sel_whe, ord, pageSize, pageIndex, out count);
            result["total"] = count;
        ...
        public void Save(HttpContext context)
        {
            ...
            foreach (Hashtable row in rows)
            {
                ...
                String id = row["RoleId"] != null ? row["RoleId"].ToString() : "";
                ...
                if (state == "added" || id == "")           //新增：id为空，或_state为added
                {
                    ...
                    row["RoleId"]=-1;
                    count += new RoleBLL().Insert(JsonHelper.Encode(row));
                }
                else if (state == "removed" || state == "deleted")  //删除
                {
                    ...
                    count += new RoleBLL().DeleteByRoleId(Convert.ToInt32(id));
                }
                else if (state == "modified" || state == "") //更新：_state为空或modified
                {
                    ...
                    count += new RoleBLL().Update(JsonHelper.Encode(row));
                }
            }
        ...
        public void Remove(HttpContext context)
        {
            ...
            {
                string id = ids[i];
                new RoleBLL().DeleteByRoleId(Convert.ToInt32(id));
            }
        }
        ...
        public void GetModel(HttpContext context)
        {
            ...
            Role role = new RoleBLL().GetByRoleId(Convert.ToInt32(id));
            String json = JsonHelper.Encode(role);
            context.Response.Write(json);
        }
