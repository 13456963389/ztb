...
namespace ZtbSoft.Web.Ajax
{
    ...
    /// FileInfo 附件管理
    /// </summary>
    public class FileInfoAjax : IHttpHandler,IRequiresSessionState
    {
		
        private DataTable dtPower = null;   //当前用户权限
        private string powerCode = enum_PowerObjectCode.FileInfo.ToString();
        ...
        public void GetList(HttpContext context)
        {
            ...
            result["data"] = new FileInfoBLL().GetPage(sel_col, sel_whe, ord, pageSize, pageIndex, out count);
            result["total"] = count;
        ...
        public void Save(HttpContext context)
        {
            ...
            foreach (Hashtable row in rows)
            {
                ...
                String id = row["FileId"] != null ? row["FileId"].ToString() : "";
                ...
                if (state == "added" || id == "")           //新增：id为空，或_state为added
                {
                    ...
                    row["FileId"]=-1;
                    count += new FileInfoBLL().Insert(JsonHelper.Encode(row));
                }
                else if (state == "removed" || state == "deleted")  //删除
                {
                    ...
                    count += new FileInfoBLL().DeleteByFileId(Convert.ToInt32(id));
                }
                else if (state == "modified" || state == "") //更新：_state为空或modified
                {
                    ...
                    count += new FileInfoBLL().Update(JsonHelper.Encode(row));
                }
            }
        ...
        public void Remove(HttpContext context)
        {
            ...
            {
                string id = ids[i];
                new FileInfoBLL().DeleteByFileId(Convert.ToInt32(id));
            }
        }
        ...
        public void GetModel(HttpContext context)
        {
            ...
            FileInfo fileInfo = new FileInfoBLL().GetByFileId(Convert.ToInt32(id));
            String json = JsonHelper.Encode(fileInfo);
            context.Response.Write(json);
        }
