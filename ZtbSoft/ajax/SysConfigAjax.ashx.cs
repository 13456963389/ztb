...
namespace ZtbSoft.Web.Ajax
{
    ...
    /// SysConfig 系统配置
    /// </summary>
    public class SysConfigAjax : IHttpHandler,IRequiresSessionState
    {
		
        private DataTable dtPower = null;   //当前用户权限
        private string powerCode = enum_PowerObjectCode.SysConfig.ToString();
        ...
        public void GetList(HttpContext context)
        {
            ...
            result["data"] = new SysConfigBLL().GetPage(sel_col, sel_whe, ord, pageSize, pageIndex, out count);
            result["total"] = count;
        ...
        public void Save(HttpContext context)
        {
            ...
            foreach (Hashtable row in rows)
            {
                ...
                String id = row["ConfigId"] != null ? row["ConfigId"].ToString() : "";
                ...
                if (state == "added" || id == "")           //新增：id为空，或_state为added
                {
                    ...
                    row["ConfigId"]=-1;
                    count += new SysConfigBLL().Insert(JsonHelper.Encode(row));
                }
                else if (state == "removed" || state == "deleted")  //删除
                {
                    ...
                    count += new SysConfigBLL().DeleteByConfigId(Convert.ToInt32(id));
                }
                else if (state == "modified" || state == "") //更新：_state为空或modified
                {
                    ...
                    count += new SysConfigBLL().Update(JsonHelper.Encode(row));
                }
            }
        ...
        public void Remove(HttpContext context)
        {
            ...
            {
                string id = ids[i];
                new SysConfigBLL().DeleteByConfigId(Convert.ToInt32(id));
            }
        }
        ...
        public void GetModel(HttpContext context)
        {
            ...
            SysConfig sysConfig = new SysConfigBLL().GetByConfigId(Convert.ToInt32(id));
            String json = JsonHelper.Encode(sysConfig);
            context.Response.Write(json);
        }
