...
namespace ZtbSoft.CostManagement.ajax
{
    /// <summary>
    /// TBBZJCostAjax 的摘要说明
    /// </summary>
    public class TBBZJCostAjax : IHttpHandler, IRequiresSessionState
    {
        private DataTable dtPower = null;   //当前用户权限
        private string powerCode = enum_PowerObjectCode.CostManagementTBBZJ.ToString();
        ...
        public void GetList(HttpContext context)
        {
            ...
            //查询字段
            string CostType = context.Request["CostType"];
            string ProjectId = context.Request["ProjectId"];
            string GoEmployeeId = context.Request["GoEmployeeId"];
            ...
            { sel_whe += " and CostType = '" + CostType + "'"; }
            if (!string.IsNullOrWhiteSpace(ProjectId))
            { sel_whe += " and ProjectId like '%" + ProjectId + "%'"; }
            if (!string.IsNullOrWhiteSpace(GoEmployeeId))
            { sel_whe += " and GoEmployeeId = '" + GoEmployeeId + "'"; }
            ...
            result["data"] = new CostManagementBLL().GetPage(sel_col, sel_whe, ord, pageSize, pageIndex, out count);
            result["total"] = count;
        ...
        public void Save(HttpContext context)
        {
            ...
            foreach (Hashtable row in rows)
            {
                ...
                String id = row["CostId"] != null ? row["CostId"].ToString() : "";
                ...
                if (state == "added" || id == "")           //新增：id为空，或_state为added
                {
                    ...
                    row["CostId"] = -1;
                    count += new CostManagementBLL().Insert(JsonHelper.Encode(row));
                }
                else if (state == "removed" || state == "deleted")  //删除
                {
                    ...
                    count += new CostManagementBLL().DeleteByCostId(Convert.ToInt32(id));
                }
                else if (state == "modified" || state == "") //更新：_state为空或modified
                {
                    ...
                    count += new CostManagementBLL().Update(JsonHelper.Encode(row));
                }
            }
        ...
        public void Remove(HttpContext context)
        {
            ...
            {
                string id = ids[i];
                new CostManagementBLL().DeleteByCostId(Convert.ToInt32(id));
            }
        }
        ...
        public void GetModel(HttpContext context)
        {
            ...
            var costManagement = new CostManagementBLL().GetByCostId(Convert.ToInt32(id));
            String json = JsonHelper.Encode(costManagement);
            context.Response.Write(json);
        }
